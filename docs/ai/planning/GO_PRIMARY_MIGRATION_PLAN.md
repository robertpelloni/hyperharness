...
- the Go HTTP layer now owns persisted local fallback writes/searches for agent-memory-backed facts, observations, user prompts, session summaries, and direct `/api/agent-memory/*` inventory/export/stats mutations through `.hypercode/agent_memory/memories.json` when the TypeScript runtime is unavailable
