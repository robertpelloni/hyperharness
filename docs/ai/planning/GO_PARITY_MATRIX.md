...
| Agent memory runtime | Partial Native Go | Go now owns persisted local add/delete/clear/search/recent/filter/export/stats/handoff/pickup fallback behavior through `.hypercode/agent_memory/memories.json`, and the adjacent saved-context fallback now also supports local context-body reads plus local registry deletion from `.hypercode/memory/contexts.json`; deeper vector-backed parity and mixed-runtime authority are still TS-first |
