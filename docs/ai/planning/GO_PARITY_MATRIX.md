...
| Swarm/squad/autodev/darwin | Partial Native Go / bridge mix | many Go routes still proxy to TS; `/api/skills/assimilate` now has a native Go fallback that creates a truthful local starter skill scaffold, Darwin routes now have a native persisted local mutation/experiment/status fallback, and AutoDev routes now have a native persisted local loop manager fallback, but richer director/LLM/research-driven Darwin, AutoDev, and assimilation parity remain incomplete |
