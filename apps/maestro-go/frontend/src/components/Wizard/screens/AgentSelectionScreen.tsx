 ...
 *
 * Features:
 * - Tiled grid view of agent logos with explicit installed/not-installed status
...
export interface AgentTile {
	id: string;
	name: string;
	supported: boolean; // Whether Maestro currently exposes this agent in the wizard
	description: string;
...
export const AGENT_TILES: AgentTile[] = [
	...
	// Remaining coming-soon agents at the bottom
	{
		id: 'gemini-cli',
		name: 'Gemini CLI',
		supported: true,
...
export function AgentSelectionScreen({ theme }: AgentSelectionScreenProps): JSX.Element {
	...
	useEffect(() => {
		if (!isDetecting) {
			// Count how many agents are selectable in the wizard
	...
	const handleKeyDown = useCallback(
		(e: React.KeyboardEvent) => {
			...
			switch (e.key) {
				...
				case ' ': {
					e.preventDefault();
					// Select the focused tile if it is exposed in the wizard
	...
	const handleTileClick = useCallback(
		(tile: AgentTile, index: number) => {
			// Supported agents remain selectable so users can configure custom paths before detection succeeds.
			if (tile.supported) {
				setSelectedAgent(tile.id as any);
			...
			{sshConnectionError ? (
				...
				<div className="flex flex-col items-center gap-4">
                  ...
                  `}
									...
									aria-label={`${tile.name} (${statusLabel.toLowerCase()})`}
								...
								>
									...
									<p className="text-xs text-center font-medium" style={{ color: theme.colors.textDim }}>
										{statusLabel}
									</p>
									...
									{isSupported && (
										<p
											className="mt-1 text-[11px] text-center leading-snug"
