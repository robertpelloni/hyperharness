...
export function UpdateCheckModal({ theme, onClose }: UpdateCheckModalProps) {
	...
	const checkForUpdates = async () => {
		...
		if (!MAESTRO_UPDATES_ENABLED) {
			setResult({
				...
				versionsBehind: 0,
				releases: [],
				releasesUrl: MAESTRO_RELEASES_URL,
				error: MAESTRO_UPDATES_DISABLED_REASON,
			});
			setLoading(false);
			return;
		}
	...
	const handleDownloadUpdate = async () => {
		setDownloadError(null);
		if (!MAESTRO_UPDATES_ENABLED) {
			setDownloadStatus({ status: 'error', error: MAESTRO_UPDATES_DISABLED_REASON });
			setDownloadError(MAESTRO_UPDATES_DISABLED_REASON);
			return;
		}
		setDownloadStatus({
			status: 'downloading',
