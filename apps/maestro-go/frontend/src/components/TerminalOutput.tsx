import React, { useRef, useEffect, useMemo, forwardRef, useState, useCallback, memo, lazy, Suspense } from 'react';
import {
	ChevronDown,
	ChevronUp,
	Trash2,
...
function MarkdownFallback({ text }: { text: string }) {
	return <div className="whitespace-pre-wrap text-sm break-words">{text}</div>;
}
						...
						>
							...
							<div className="whitespace-pre-wrap text-sm break-words">
								{isAIMode && !markdownEditMode ? (
									<Suspense fallback={<MarkdownFallback text={log.text} />}>
							...
							<div>
								...
								>
									{isTerminal && log.source !== 'user' ? (
										...
										<Suspense fallback={<MarkdownFallback text={displayText} />}>
											<LazyMarkdownRenderer
												content={displayText}
												theme={theme}
												onCopy={copyToClipboard}
							...
							<div>
								...
								>
									{isTerminal && log.source !== 'user' ? (
										...
										<Suspense fallback={<MarkdownFallback text={filteredText} />}>
											<LazyMarkdownRenderer
