	...
	useEffect(() => {
		...
		const loadSessionsAndGroups = async () => {
			try {
				...
				// Mark sessions as loaded once the core session/group state is ready.
				// Group chat enumeration can lag or fail without blocking the whole app
				// behind the splash screen.
				setSessionsLoaded(true);

				// Load group chats after the splash gate is opened.
				try {
