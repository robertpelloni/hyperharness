...
func TestExecuteToolCallHandlesUnknownAndErrorPaths(t *testing.T) {
	unknown := executeToolCall(nil, openai.ToolCall{Function: openai.FunctionCall{Name: "missing"}}, nil)
	...
	result := executeToolCall(registry, openai.ToolCall{Function: openai.FunctionCall{Name: "demo", Arguments: `{"x":1}`}}, nil)
	if result != "partial" {
	...
	result = executeToolCall(registry, openai.ToolCall{Function: openai.FunctionCall{Name: "demo", Arguments: `not-json`}}, nil)
