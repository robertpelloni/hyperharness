...
const CACHE_BASE_PATH = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'thedotmack', 'borg-extension');
...
try {
  ...
  if (os.platform() === 'win32') {
    // Windows: use robocopy (available on all modern Windows)
    // robocopy /MIR mirrors source to dest, /XD excludes dirs, /XF excludes files
    try {
      execSync(
        `robocopy "${rootDir}" "${INSTALLED_PATH}" /MIR /XD .git node_modules dist datasets "Auto Run Docs" .cursor .idea private /XF bun.lock package-lock.json *.log .DS_Store .env .env.local *.tmp *.temp .install-version .mcp.json /NFL /NDL /NJH /NJS /NC /NS`,
        { stdio: 'inherit' }
      );
    } catch (e) {
      // robocopy returns non-zero exit codes for success (1 = files copied, 2 = extra files)
      // Only exit codes >= 8 indicate actual errors
      if (e.status >= 8) {
        throw e;
      }
    }
  ...
  if (os.platform() !== 'win32') {
    ...
    execSync('npm install', { cwd: INSTALLED_PATH, stdio: 'inherit' });
  }
  ...
  if (os.platform() === 'win32') {
    try {
      execSync(
        `robocopy "${pluginDir}" "${CACHE_VERSION_PATH}" /MIR /XD .git node_modules /XF *.log .DS_Store /NFL /NDL /NJH /NJS /NC /NS`,
        { stdio: 'inherit' }
      );
    } catch (e) {
