...
/**
 * OpenCode Plugin for HyperCode (formerly Claude-Mem)
 */
export const HyperCodePlugin: Plugin = async (ctx) => {
          ...
          try {
             ...
             if (Array.isArray(session.messages)) {
                 session.messages.push({
                     role: "system",
                     content: `[HyperCode] Memory Active. Previous Context:\n${context}`
                 });
             }
          } catch (e) {
              console.error("[HyperCode] Failed to inject context", e);
          }
      }
    },
