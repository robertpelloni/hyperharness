...
func TestProcessSlashCommandTreePanePreviewQuickToggle(t *testing.T) {
	...
	updated := mdl.(model)
	if updated.browserPanePreview {
		t.Fatal("expected quick preview toggle to disable preview")
	}
	mdl, _ = ProcessSlashCommand("/tree-pane-preview-toggle", &updated)
	updated = mdl.(model)
	if !updated.browserPanePreview {
		t.Fatal("expected second quick preview toggle to enable preview")
	}
}
...
func TestProcessSlashCommandDashboardToggle(t *testing.T) {
	...
	mPtr, _ := ProcessSlashCommand("/dashboard", m)
	m2 := mPtr.(model)
	if !m2.dashboardActive {
		t.Errorf("Expected dashboardActive to be true after /dashboard command")
	}

	mPtr, _ = ProcessSlashCommand("/dashboard", &m2)
	m3 := mPtr.(model)
	if m3.dashboardActive {
		t.Errorf("Expected dashboardActive to be false after second /dashboard command")
	}
}
