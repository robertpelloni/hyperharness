  ...
  const fetchAgents = () => {
    ...
    fetch('/api/state')
      ...
      .then(data => {
        ...
        setAgents([
          { url: 'http://localhost:3001', card: { name: 'AIOS Meta-Orchestrator', version: '0.3.0', skills: [{ name: 'Task Orchestration' }] }, healthy: true },
          { url: 'https://agent-east.aios.dev', card: { name: 'Security-Audit-Bot', version: '1.2.0', skills: [{ name: 'Vulnerability Scan' }] }, healthy: true }
        ]);
        setLoading(false);
      })
      .catch(err => {
