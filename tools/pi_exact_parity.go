...
func (r *Registry) registerPiExactTools() {
	...
	r.Tools = append(r.Tools, Tool{
		...
		// Execute handles the 'bash' tool execution, mapping parameters to the underlying foundation functionality.
		Execute: func(args map[string]interface{}) (string, error) {
	...
	r.Tools = append(r.Tools, Tool{
		...
		// Execute handles the 'ls' tool execution, mapping parameters to the underlying foundation functionality.
		Execute: func(args map[string]interface{}) (string, error) {
...
func (r *Registry) registerHypercodeTools() {
	...
	r.Tools = append(r.Tools, Tool{
		...
		Execute: func(args map[string]interface{}) (string, error) {
			...
			if cmRaw, ok := args["_context_manager"]; ok {
				if cm, ok := cmRaw.(*contextmgr.Manager); ok {
					ctxMgr = cm
				}
			}
			if ctxMgr == nil {
				ctxMgr = contextmgr.NewManager(100, 200000)
			}
			...
			case "status":
				...
				var memStatsStr string
				sqlStore, err := getSQLiteMemoryStore()
				if err == nil && sqlStore != nil {
					count, _ := sqlStore.Count()
					memStatsStr = fmt.Sprintf("SQLite Memory Entries: %d", count)
				} else {
					kb, err := getKnowledgeBase()
					if err == nil && kb != nil {
						stats := kb.Stats()
...
func registerAllParityTools(r *Registry) {
	...
	registerAdvancedParityTools(r)
	registerHermesIIParityTools(r)
	registerPowerShellTools(r)
	r.registerTabbyParityTools()
	r.registerWarpParityTools()
	r.registerWaveParityTools()
	r.registerHermesParityTools()
	r.registerAntigravityParityTools()
	r.registerCodexParityTools()
	r.registerIngestDataTool()
	RegisterAutoGeneratedTools(r)
}
