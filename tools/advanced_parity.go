...
func registerAdvancedParityTools(r *Registry) {
	tools := []Tool{
		// OpenCode's apply_patch tool
		{
			...
			// Execute handles the 'apply_patch' tool execution, mapping parameters to the underlying foundation functionality.
		...
		// OpenCode's batch execution tool
		{
			...
			Execute: func(args map[string]interface{}) (string, error) {
				rawInvocations, ok := args["invocations"].([]interface{})
				if !ok {
					return "", fmt.Errorf("invocations must be an array of objects")
				}

				var wg sync.WaitGroup
				var mu sync.Mutex
				results := make([]string, len(rawInvocations))
				var hasErr error

				// We need the global registry to call tools from within a tool
				// Assuming _registry is injected during Execution like _context_manager
				registryObj, _ := args["_registry"].(*Registry)

				for i, invRaw := range rawInvocations {
					inv, ok := invRaw.(map[string]interface{})
					if !ok {
						continue
					}
					toolName := GetStr(inv, "tool_name")
					if toolName == "" {
						toolName = GetStr(inv, "name") // fallback
					}
					paramsRaw := inv["parameters"]
					...
					if toolName == "batch" {
						results[i] = fmt.Sprintf("Error: nested batch calls are not allowed")
						continue
					}

					if registryObj == nil {
						results[i] = fmt.Sprintf("Error: batch tool requires registry context")
						continue
					}

					wg.Add(1)
					go func(index int, name string, p map[string]interface{}) {
						defer wg.Done()
						
						// Pass down context objects if present
						if cm, ok := args["_context_manager"]; ok {
							p["_context_manager"] = cm
						}
						if pjm, ok := args["_job_manager"]; ok {
							p["_job_manager"] = pjm
						}
						p["_registry"] = registryObj

						tool, ok := registryObj.Find(name)
						if !ok {
							mu.Lock()
							results[index] = fmt.Sprintf("Tool %s not found in registry", name)
							hasErr = fmt.Errorf("tool not found")
							mu.Unlock()
							return
						}
						
						res, err := tool.Execute(p)
						
						mu.Lock()
						defer mu.Unlock()
						if err != nil {
							results[index] = fmt.Sprintf("Tool %s failed: %v", name, err)
							hasErr = err
						} else {
							results[index] = fmt.Sprintf("=== Tool %s Result ===\n%s", name, res)
						}
					}(i, toolName, params)
				}
				...
				if hasErr != nil {
					return strings.Join(results, "\n\n"), fmt.Errorf("batch execution had errors")
				}
		...
		// Crush's LSP diagnostics tool
		{
			...
			// Execute handles the 'lsp_diagnostics' tool execution, mapping parameters to the underlying foundation functionality.
			Execute: func(args map[string]interface{}) (string, error) {
				filePath := GetStr(args, "file_path")
				if filePath != "" {
		...
		// Crush's LSP restart tool
		{
			...
			// Execute handles the 'lsp_restart' tool execution, mapping parameters to the underlying foundation functionality.
			Execute: func(args map[string]interface{}) (string, error) {
		...
		// Crush's agentic fetch tool
		{
			...
			// Execute handles the 'agentic_fetch' tool execution, mapping parameters to the underlying foundation functionality.
			Execute: func(args map[string]interface{}) (string, error) {
				url := GetStr(args, "url")
				prompt := GetStr(args, "prompt")
				if url != "" {
