...
// compacting, reranking, and semantic chunking.
// Ported from hypercode/go/internal/ctxharvester/harvester.go
//
// WHAT: Harvests context from files, terminal, git, conversation; manages token budgets
// WHY: LLM context windows are limited; relevance-ranked pruning maximizes quality
// HOW: Semantic chunking → relevance scoring → time decay → token-budgeted retrieval
...
// DefaultHarvestConfig returns sensible defaults.
func DefaultHarvestConfig() HarvestConfig {
	return HarvestConfig{
		MaxTokenBudget:     128000,
		PruneThreshold:     0.1,
		CompactAfterMs:     30 * 60 * 1000,
		RerankOnAccess:     true,
		ChunkSize:          300,
		ChunkOverlap:       30,
...
func (ch *ContextHarvester) GetConfig() HarvestConfig { return ch.config }
...
func (ch *ContextHarvester) Harvest(source ContextSource, content string, metadata map[string]interface{}) []*ContextChunk {
	...
	for i, text := range textChunks {
		ch.nextID++
		id := fmt.Sprintf("ctx_%d_%d_%d", now, ch.nextID, i)
		chunk := &ContextChunk{
			ID:             id,
			Source:         source,
			Content:        text,
...
// scoredEntry is used for ranking during retrieval.
type scoredEntry struct {
	chunk *ContextChunk
	score float64
}

// Retrieve returns the most relevant chunks for a query within a token budget.
...
// Prune removes chunks below the relevance threshold.
...
// Compact merges adjacent old chunks from the same source.
func (ch *ContextHarvester) Compact() int {
	...
	for _, chunks := range bySource {
		...
		newChunk := &ContextChunk{
			ID:             fmt.Sprintf("ctx_merged_%d_%d", now, ch.nextID),
...
func (ch *ContextHarvester) ChunkCount() int {
	ch.mu.RLock()
	defer ch.mu.RUnlock()
