...
// ExportAll dumps all MemoryEntries from the SQLite database.
func (s *SQLiteMemoryStore) ExportAll() ([]MemoryEntry, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// ImportEntries imports a list of MemoryEntries into the database.
func (s *SQLiteMemoryStore) ImportEntries(entries []MemoryEntry) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for _, entry := range entries {
		...
		if err != nil {
			return fmt.Errorf("failed to import entry %s: %w", entry.ID, err)
		}
	}
...
// SearchScoped searches using FTS5 but filters by scope.
func (s *SQLiteMemoryStore) SearchScoped(query, scope string, limit int) ([]MemoryEntry, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	// Fallback to LIKE if FTS fails or is empty
	if err != nil {
		likeQuery := "%" + query + "%"
		rows, err = s.db.Query(`
			SELECT id, content, type, tags, source, title, scope, created_at, updated_at
	...
	// If FTS didn't error but gave no results, try LIKE
