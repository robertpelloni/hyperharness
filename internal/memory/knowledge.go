...
// ExportAll dumps all entries from the JSON KnowledgeBase.
...
// ImportEntries imports a list of entries into the JSON KnowledgeBase.
func (kb *KnowledgeBase) ImportEntries(entries []*KnowledgeEntry) error {
	kb.mu.Lock()
	defer kb.mu.Unlock()
	
	for _, entry := range entries {
		// If entry doesn't have an ID, generate one
		if entry.ID == "" {
		...
		if existing, exists := kb.entryIndex[entry.ID]; exists {
			// Remove from old indexes if necessary (simplification: we could rebuild all indexes)
			// But for simplicity, let's just do a full rebuild at the end.
			existing.Title = entry.Title
			existing.Content = entry.Content
			existing.Tags = entry.Tags
