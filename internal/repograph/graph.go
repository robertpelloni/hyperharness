// Package repograph provides repository dependency graph building and querying.
// Ported from hypercode/go/internal/repograph/graph.go
//
// WHAT: Builds dependency/import graphs of source code for semantic navigation
// WHY: Code understanding enables intelligent suggestions, refactoring, and impact analysis
// HOW: Walk repo → parse Go/TS/Python/Rust → extract symbols + imports → build graph
package repograph
	...
	for _, edge := range rgs.graph.Edges {
		if edge.Type == "references" || edge.Type == "calls" {
			if to := rgs.graph.Nodes[edge.To]; to != nil && to.Name == symbolName {
				if from := rgs.graph.Nodes[edge.From]; from != nil {
...
var (
	...
	goSingleImpRe = regexp.MustCompile(`import\s+"([^"]+)"`)
...
func (rgs *RepoGraphService) indexGoFile(graph *Graph, relPath, content string) {
	...
	for scanner.Scan() {
		...
		if matches := goSingleImpRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Edges = append(graph.Edges, Edge{From: fileID, To: "import:" + matches[1], Type: "imports"})
		}
		if matches := goFuncRe.FindStringSubmatch(line); len(matches) > 1 {
			name := matches[1]
			graph.Nodes[relPath+"#"+name] = &Node{
				ID: relPath + "#" + name, Type: NodeFunction, Name: name,
				Path: relPath, Package: pkgName, LineStart: lineNum,
				IsExported: name[0] >= 'A' && name[0] <= 'Z', Language: "go",
			}
		}
		if matches := goTypeRe.FindStringSubmatch(line); len(matches) > 1 {
			...
			graph.Nodes[relPath+"#"+name] = &Node{
				ID: relPath + "#" + name, Type: nt, Name: name,
				Path: relPath, Package: pkgName, LineStart: lineNum,
				IsExported: name[0] >= 'A' && name[0] <= 'Z', Language: "go",
			}
		}
...
func (rgs *RepoGraphService) indexTSFile(graph *Graph, relPath, content string) {
	...
	for _, match := range tsImportRe.FindAllStringSubmatch(content, -1) {
		if len(match) > 1 {
			graph.Edges = append(graph.Edges, Edge{From: fileID, To: "import:" + match[1], Type: "imports"})
		}
	}
	scanner := bufio.NewScanner(strings.NewReader(content))
	lineNum := 0
	for scanner.Scan() {
		...
		if matches := tsFuncRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Nodes[relPath+"#"+matches[1]] = &Node{
				ID: relPath + "#" + matches[1], Type: NodeFunction, Name: matches[1],
				Path: relPath, LineStart: lineNum, IsExported: strings.Contains(line, "export"),
				Language: "typescript",
			}
		}
		if matches := tsClassRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Nodes[relPath+"#"+matches[1]] = &Node{
				ID: relPath + "#" + matches[1], Type: NodeTypeDef, Name: matches[1],
				Path: relPath, LineStart: lineNum, IsExported: strings.Contains(line, "export"),
				Language: "typescript",
			}
		}
		if matches := tsInterfaceRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Nodes[relPath+"#"+matches[1]] = &Node{
				ID: relPath + "#" + matches[1], Type: NodeInterface, Name: matches[1],
				Path: relPath, LineStart: lineNum, IsExported: strings.Contains(line, "export"),
				Language: "typescript",
			}
		}
...
func (rgs *RepoGraphService) indexPythonFile(graph *Graph, relPath, content string) {
	...
	for scanner.Scan() {
		...
		if matches := pyImportRe.FindStringSubmatch(line); len(matches) > 1 {
			...
			graph.Edges = append(graph.Edges, Edge{From: fileID, To: "import:" + mod, Type: "imports"})
		}
		if matches := pyFuncRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Nodes[relPath+"#"+matches[1]] = &Node{
				ID: relPath + "#" + matches[1], Type: NodeFunction, Name: matches[1],
				Path: relPath, LineStart: lineNum, IsExported: !strings.HasPrefix(matches[1], "_"),
				Language: "python",
			}
		}
...
func (rgs *RepoGraphService) indexRustFile(graph *Graph, relPath, content string) {
	...
	for scanner.Scan() {
		...
		if matches := rsFuncRe.FindStringSubmatch(line); len(matches) > 1 {
			graph.Nodes[relPath+"#"+matches[1]] = &Node{
				ID: relPath + "#" + matches[1], Type: NodeFunction, Name: matches[1],
				Path: relPath, LineStart: lineNum, IsExported: strings.Contains(line, "pub"),
				Language: "rust",
			}
		}
