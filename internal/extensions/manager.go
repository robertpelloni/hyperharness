...
// InstallFromSmithery installs an MCP server from a Smithery registry.
func (m *Manager) InstallFromSmithery(serverName string, config map[string]interface{}) error {
	// Query Smithery API
	apiURL := fmt.Sprintf("https://api.smithery.ai/v1/servers/%s", serverName)
	resp, err := http.Get(apiURL)
	if err != nil {
		return fmt.Errorf("failed to fetch from smithery: %w", err)
	}
	defer resp.Body.Close()
	
	if resp.StatusCode != http.StatusOK {
		// Just register it loosely if not found for mock resilience
		ext := &Extension{
			Name:        serverName,
			Type:        TypeMCPStdio,
			Description: fmt.Sprintf("MCP server: %s (unverified via API)", serverName),
			Enabled:     true,
			AutoStart:   true,
	...
	if err := json.Unmarshal(body, &data); err != nil {
		return fmt.Errorf("failed to decode smithery response: %w", err)
	}
